...
<template>
  <div
    :class="[(active) ? 'canvasBox': 'canvasBoxDeactive']"
    :style="
      `
  ...
  methods: {
    ...
    loop() {
      if (this.ctx !== undefined) {
        ...
        if (this.seeds.length == 0 && this.particles.length == 0 && this.finished) {
          this.wipeCanvas()
        }
    ...
    _fireWorkRatio(maxPoints, points) {
      const ratio = 1.3 - (points * 1.0) / maxPoints;
      return 2000 * ratio;
    },
    createFireWork(maxPoints, points) {
      this.active = true;
      const pushFireWork = () => {
        const seed = this.Seed(
          this.randomInt(0, this.width),
          this.height - 20,
      ...
      const interval = window.setInterval(
        pushFireWork.bind(this),
        this._fireWorkRatio(maxPoints, points)
      ); // TODO stop firework after 10 seconds
      window.setTimeout(
        (() => {
